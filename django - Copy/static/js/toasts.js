(function () {
  ...
  // Bỏ early return ở đây để các hàm global vẫn được tạo.
  // if (!root) return;
  ...
  // Export globally so it can be called from onclick events
  window.showAdsToast = function(level, text, delay) {
    if (!root) {
      // Lazy init root if missing (in case of dynamic loading)
      root = document.createElement('div');
      root.id = "ads-toast-root";
      document.body.appendChild(root);
    }
    showToast(mapLevel(level), text, delay || 3500);
  };
